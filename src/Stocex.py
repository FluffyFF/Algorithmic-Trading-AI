...
def score_sentiment_for_mentioned_tickers(news_df, known_companies, mentioned_tickers):
    ...
    for ticker, records in sentiment_records.items():
        ...
        if avg_score >= 0.5:  # ✅ Only keep perfect scores
...
os.makedirs("historical_price_data", exist_ok=True)
for ticker, df in historical_price_data.items():
    df.to_csv(f"historical_price_data/{ticker}_price_history.csv", index=False)

# ✅ Create a 'stocex_data' folder and move files into it
os.makedirs("stocex_data", exist_ok=True)
shutil.move("sentiment_summary.csv", "stocex_data/sentiment_summary.csv")
shutil.move("news_headlines.csv", "stocex_data/news_headlines.csv")
shutil.copytree("historical_price_data", "stocex_data/historical_price_data", dirs_exist_ok=True)
...
# # Code to copy files to github repo and update them daily


# # Copy your generated files to /data
# !cp sentiment_summary.csv algorithmic-trading-ai/analysis/data/
# !cp news_headlines.csv algorithmic-trading-ai/analysis/data/
# !cp historical_price_data/*.csv algorithmic-trading-ai/analysis/data/

# # Commit & push
# %cd algorithmic-trading-ai
# !git config --global user.email "sanjukohli27@gmail.com"
# !git config --global user.name "Sanjeevteja P"
# !git add analysis/data/
# !git commit -m "🔄 Auto-updated data for today"
# !git push
...
# Step 3: Forecast
forecast_df = forecast_with_timegpt(series_daily, horizon=3)
...
# Cache News
if os.path.exists("news_yesterday.csv"):
    news_df = pd.read_csv("news_yesterday.csv")
else:
    news_df = fetch_yesterdays_news()
    news_df.to_csv("news_yesterday.csv", index=False)

# Batch yfinance fetch (pseudo code)
tickers = ["AAPL", "TSLA", "MSFT"]
data = yf.download(tickers, period="30d", interval="5m", group_by='ticker')
